...
export async function getActionConfig() {
  ...
  for (const dir of dirs) {
    const dirFiles = []
    for await (const file of glob("**/*.json", { cwd: path.join(process.cwd(), folderPath, dir) })) {
      dirFiles.push(file)
    }
    ...
    const configFile = await parseConfigFile(path.join(process.cwd(), folderPath, dir, latestConfigFile))
